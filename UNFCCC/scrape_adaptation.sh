...
mkdir -p tmp
mkdir -p pdfs
echo "url|id|symbol|title|authors|en_pdf_url|abstract|meeting|doctype|topics|keywords|countries|pubdate|year" > unfccc_adaptation_metadatas.csv
...
for search in "adaptation" "vulnerability" "climate+impact" "adaptative" "vulnerable"; do
      ...
      grep '"adlibsearch_text_container_right' "tmp/dataset-$id.html" | sed 's#\s*<\/\?div[^>]*>\s*##g' | while read line; do
        case "$ct" in
         ...
         12) date=`echo $line | sed 's#^.* \([0-9]\+\)/\([0-9]\+\)/\([0-9]\{4\}\).*$#\3-\2-\1#'`
             echo "$fields|$date|$annee" >> unfccc_adaptation_metadatas.csv;;
